...
def find_all_neighbours(cube):
    neighbours = [(cube[0] + x, cube[1] + y, cube[2] + z)
                  for x in range(-1, 2)
                  for y in range(-1, 2)
                  for z in range(-1, 2)]
    neighbours.remove((cube[0], cube[1], cube[2]))
    return neighbours


def create_cubes(cubes_lines):
    cubes = {(x, y, 0): cubes_lines[x][y] for x in range(len(cubes_lines)) for y in range(len(cubes_lines))}
    return cubes
